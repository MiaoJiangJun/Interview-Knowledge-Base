...
下面这组场景按“从基础到高阶”安排。你可以按顺序练，也可以每次挑一个练。建议每个场景都先自己做一遍，再回头看本页命令表查漏补缺。

### 场景 1：新手热身，先把光标和插入模式练熟
...
1. 用 `gg` 回到第一行。
2. 用 `j` 到第三行，再用 `l` 把光标移动到第三个 `c`。
3. 用 `i` 在当前位置前插入 `X`，再按 `Esc` 返回普通模式。
4. 用 `h`、`l` 左右移动确认位置。
5. 用 `0` 回行首，用 `$` 到行尾。
6. 用 `k` 回到第二行，再用 `a` 在行尾后追加 `-tail`。
7. 用 `A` 在第一行末尾追加 `-end`。
8. 用 `o` 在当前行下方新建一行输入 `middle`，再用 `O` 在上方新建一行输入 `top`。

这一组重点复习：`i` `Esc` `h` `j` `k` `l` `gg` `G` `0` `$` `a` `A` `o` `O`
...
1. 把光标放到 `beta` 行，用 `yy` 复制该行。
2. 用 `p` 把复制内容粘贴到下一行，再用 `P` 粘贴到上一行。
3. 在 `gamma` 行用 `dd` 删除整行。
4. 在 `alpha` 行用 `x` 连续删两个字符，再用 `X` 删除光标前一个字符。
5. 用 `u` 撤销三次，再用 `Ctrl+r` 重做两次。
6. 用 `:w` 保存，再分别练一次 `:q`、`:wq`、`ZZ`、`:q!` 的退出逻辑。

这一组重点复习：`yy` `p` `P` `dd` `x` `X` `u` `Ctrl+r` `:w` `:q` `:wq` `ZZ` `:q!`
...
```text
int main return value count total
```

任务：

1. 用 `w` 在单词开头之间跳转，用 `b` 向后跳。
2. 用 `e` 移动到单词末尾，用 `ge` 回到上一个单词末尾。
3. 在 `return` 上练 `dw` 删除到下一个单词，撤销后再练 `db`。
4. 在 `value` 上练 `cw`，把它改成 `result`。
5. 在 `count` 上练 `yw` 复制一个单词，并在行尾用 `p` 粘贴。
6. 把光标放到行中间，依次练 `d$`、`D`、`d0`，观察删除范围。
7. 再用 `y$` 复制到行尾。

这一组重点复习：`w` `b` `e` `ge` `dw` `db` `cw` `yw` `d$` `D` `d0` `y$`
...
1. 用 `cc` 删除第二行并重新输入 `line new data`。
2. 用 `C` 把第三行从当前光标处改写为 `release info`。
3. 在第一行用 `s` 改一个字符，再用 `S` 整行重写。
4. 任选一行，用 `r` 替换单个字符。
5. 用 `~` 把几个字母切换大小写。
6. 找一个刚做过的修改，连续按 `.` 重复它。
7. 输入两行短句，再用 `J` 合并成一行。
...
1. 用 `/error` 向下搜索，再用 `n`、`N` 在结果间切换。
2. 用 `?apple` 向上搜索。
3. 把光标放在 `log` 上，分别练 `*` 和 `#`。
4. 在当前行执行 `:s/error/fault`。
5. 在当前行执行 `:s/log/record/g`。
6. 对全文执行 `:%s/apple/fruit/g`。
7. 再执行 `:%s/log/output/gc`，逐个确认替换。
8. 替换结束后用 `:nohlsearch` 取消高亮。

这一组重点复习：`/pattern` `?pattern` `n` `N` `*` `#` `:s/old/new` `:s/old/new/g` `:%s/old/new/g` `:%s/old/new/gc` `:nohlsearch`
...
1. 新建 `nav.c`
2. 粘贴一段 30 行以上的 C 或 Python 代码。

任务：

1. 用 `Ctrl+d`、`Ctrl+u` 做半页滚动。
2. 用 `Ctrl+f`、`Ctrl+b` 做整页滚动。
3. 用 `H`、`M`、`L` 跳到当前屏幕的上、中、下位置。
4. 用 `{`、`}` 在函数块或空行段落间跳转。
5. 用 `nG` 跳到指定行，例如 `15G`。
6. 用 `%` 在成对括号、方括号、花括号之间跳转。
7. 用 `^` 回到当前行第一个非空字符。

这一组重点复习：`Ctrl+u` `Ctrl+d` `Ctrl+f` `Ctrl+b` `H` `M` `L` `{` `}` `nG` `%` `^`
...
1. 用 `v` 选中部分字符后删除或复制。
2. 用 `V` 一次选中两到三整行，再统一缩进。
3. 对选中区域练 `>>` 和 `<<`。
4. 在一段缩进混乱的代码上练 `==` 自动缩进。
5. 用 `J` 把两行合并，再撤销，观察效果。
...
1. 创建 `main.c`、`util.c`、`note.md` 三个文件。
2. 每个文件随便写两三行不同内容。

任务：

1. 在 Vim 中用 `:e filename` 打开另一个文件。
2. 用 `:ls` 查看缓冲区列表。
3. 用 `:bn`、`:bp` 在缓冲区间切换。
4. 用 `:bd` 关闭一个缓冲区。
5. 用 `:sp` 和 `:vsp` 打开水平、垂直分屏。
6. 用 `Ctrl+w w` 在窗口间轮转。
7. 用 `Ctrl+w h/j/k/l` 按方向切换窗口。
8. 练一次 `Ctrl+w =`、`Ctrl+w _`、`Ctrl+w |` 调整窗口尺寸。
9. 用 `:tabnew` 打开新标签页，再用 `gt`、`gT` 切换。
10. 用 `:saveas backup.txt` 另存为，再用 `Ctrl+g` 查看文件信息。

这一组重点复习：`:e filename` `:ls` `:bn` `:bp` `:bd` `:sp` `:vsp` `Ctrl+w w` `Ctrl+w h/j/k/l` `Ctrl+w =` `Ctrl+w _` `Ctrl+w |` `:tabnew` `gt` `gT` `:saveas filename` `Ctrl+g`

### 场景 9：外部命令、重载文件、跳文件

准备：

1. 新建 `names.txt`，输入几行乱序名字。
2. 同目录再建一个 `target.txt`，在 `names.txt` 中写上一行 `target.txt`。

任务：

1. 在 Vim 里执行 `:!command`，例如 `:!dir` 或 `:!ls` 查看目录内容。
2. 对 `names.txt` 执行 `:%!sort` 排序全文。
3. 把光标放到 `target.txt` 上，用 `gf` 直接跳转打开。
4. 修改文件但不保存，用 `:e!` 放弃修改并重载。
5. 保存后再用 `:edit` 重新读取当前文件。
6. 在英文单词上按 `K` 查看帮助文档或 man 信息。
7. 如果当前是 Python 文件，试一次 `:!python %` 运行脚本。

这一组重点复习：`:!command` `:%!sort` `gf` `:edit` `:e!` `K` `:!python %`

### 场景 10：块编辑、大小写、替换模式、数字增减
...
1. 用 `Ctrl+v` 进入可视块模式，选中四行的 `name` 前方列。
2. 按 `I` 批量插入 `// `，再按 `Esc` 观察块插入效果。
3. 用 `R` 进入替换模式，把一段字符覆盖改写。
4. 在单词上练 `gU` 转大写、`gu` 转小写。
5. 把光标放到数字上，分别用 `Ctrl+a`、`Ctrl+x` 调整数值。
6. 用 `ga` 查看某个字符的编码。
7. 用 `:set number` 打开行号，再用 `:set nonumber` 关闭。

这一组重点复习：`Ctrl+v` `R` `gU` `gu` `Ctrl+a` `Ctrl+x` `ga` `:set number`
...
1. 用 `qa` 开始录制宏到寄存器 `a`。
2. 给当前行行首加上 `- `，移动到下一行，然后按 `q` 停止录制。
3. 用 `@a` 在下一行执行一次，再用 `[count]` 形式例如 `2@a` 执行多次。
4. 用 `:reg` 查看寄存器内容。
5. 复制一行到系统剪贴板，练 `"*y`，再找个位置用 `"*p` 粘贴回来。
6. 任选一处文本，练一次“组合命令”，例如 `dG`、`y$`、`guw`。
7. 再用 `:normal I# ` 给多行批量加前缀。

这一组重点复习：`q[a-z]` `q` `@[a-z]` `:reg` `"*y` `"*p` `[count]` `组合命令` `:normal`
...
1. 选中一段内容，用 `zf` 创建折叠。
2. 用 `zc` 关闭折叠，再用 `zo` 打开折叠。
3. 在文件里连续做几次修改，然后用 `g;` 跳到上一个修改位置。
4. 再用 `g,` 跳到下一个修改位置。
5. 先从一个位置跳到另一个位置，例如用搜索或 `G`，再用 `Ctrl+o` 返回旧位置。
6. 再用 `Ctrl+i` 前进到刚才的位置。

这一组重点复习：`zf` `zo` `zc` `g;` `g,` `Ctrl+o` `Ctrl+i`
...
1. 用 `:g/DEBUG/d` 删除所有调试日志。
2. 撤销后，再用 `:v/ERROR/d` 只保留错误日志。
3. 用 `/ERROR` 搜索并逐条查看结果。
4. 对保留下来的错误行用 `yy`、`p` 复制一份做备份。
5. 用 `:help :g` 查看全局命令帮助。
6. 再试一次 `:help ctrl-v` 或 `:help yank`，熟悉帮助系统。

这一组重点复习：`:g/pattern/d` `:v/pattern/d` `/pattern` `yy` `p` `:help` `:help topic`

### 建议练习顺序

1. 第一天只做场景 1 到场景 4，把普通模式、插入模式、删除复制这些动作练熟。
2. 第二天做场景 5 到场景 8，把搜索替换、窗口切换、多文件操作练熟。
3. 第三天做场景 9 到场景 13，把宏、寄存器、折叠、批处理命令串起来。
4. 每练完一个场景，自己再加一个“小变体”，例如把 `apple` 改成你工作里常见的单词，把日志改成你熟悉的格式。

如果你愿意，我下一步可以继续帮你把这些场景改成“每日 15 分钟 Vim 复习计划”，或者直接整理成“打卡清单版”。
